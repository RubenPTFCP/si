...
class LogisticRegression:
    ...
    def fit(self, dataset: Dataset) -> 'LogisticRegression':
        ...
        for i in range(self.max_iter):
            self.cost_history[i] = self.cost(dataset=dataset) # armazena no dicionário o valor que está no dataset

            if i > 1 and (self.cost_history[i - 1] - self.cost_history[i] < 1): # threshold = 1 , verifica se o elemento na posição anterior com a posição atual é menor que o threshold
                break
...
if __name__ == '__main__':
    ...
    dataset_ = read_csv("C:\\Users\\ruben\\PycharmProjects\\SI\\si\\datasets\\breast-bin.csv", ",", features = True, label = True)
    dataset_train, dataset_test = train_test_split(dataset_, test_size=0.2)

    # fit the model
    model = LogisticRegression(l2_penalty=1, alpha=0.001, max_iter=1500)
